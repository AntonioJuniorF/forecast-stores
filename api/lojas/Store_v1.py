...
class Store1:
    ...
    def Eliminated_stores(self,df):
        ...
        df_aux = df.groupby('date').store.count().to_frame()
        df_aux = df[(df['date'] >='2014-07-01') & (df['date'] <='2014-12-31')]

        vet = np.in1d(df['store'].unique(),df_aux['store'].unique() , invert=True)

        store = df['store'].unique()[vet]
  
        for i in range(len(store)):
            df = df.drop(df[df['store'] == store[i]].index.values)
